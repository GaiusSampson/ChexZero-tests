...
def preprocess(img, desired_size=320):
    ...
    # img = img.resize(new_size, Image.ANTIALIAS)        //broken in new version replaced with line below
...
""" OLD FUNCTION OUT OF MEMORY ISSUES
...
def img_to_hdf5(cxr_paths: List[Union[str, Path]], out_filepath: str, resolution=320, chunk_size=1000): 
        """
        Convert directory of images into a .h5 file given paths to all 
        images. Processes in chunks to avoid memory issues.
        """
        dset_size = len(cxr_paths)
        failed_images = []
        
        with h5py.File(out_filepath, 'w') as h5f:
            # Create dataset with chunking enabled for better memory management
            img_dset = h5f.create_dataset(
                'cxr', 
                shape=(dset_size, resolution, resolution),
                dtype='uint8',  # Specify dtype to save memory
                chunks=(1, resolution, resolution),  # Process one image at a time
            )
            ...
            for idx, path in enumerate(tqdm(cxr_paths)):
                try: 
                    ...
                    # Convert to numpy array and write directly
        ...
        print(f"{len(failed_images)} / {len(cxr_paths)} images failed to be added to h5.")
        if failed_images:
            print("First 10 failures:", failed_images[:10])
